...
export async function createEmployee(
  payload: InputEmployeeSchemaType
...
export async function updateEmployee(
  payload: UpdateEmployeeSchemaType
): Promise<ServerActionReturn<void>> {
  ...
  try {
    ...
    if (e instanceof PrismaClientKnownRequestError) {
      if (e.code === "P2002") {
        ...
        return errorResponse("Employee tidak ditemukan.", "NOT_FOUND");
      }
    }
    return errorResponse(
...
export async function getEmployeeByUsernameAction(username: string) {
  return await prisma.user.findUnique({
    where: { username, role: "employee" },
  });
}
